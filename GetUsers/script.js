                        ...
                        for (var i = 0; i < roles.length; i++) { // For loop going through the roles array of the users array
                            if (roles[i].groups !== undefined) { // Checks if there are any groups
                                ...
                                for (let item of name_arr) { // Loop through the array of names
                                    ...
                                    let group = await readAsset("group", item); // Reads the group asset
                                    let group_arr = group.group.users.split(';');
                                    for (let member of group_arr) { // Loops through each member of the group
                                        htmlText += `<ul><li>${member}</li></ul>`; // Outputs each member as a sublist
                                    }
                                }
...
async function getSetUsersAndGroups() { // Get all the users, adds them to a set
    document.getElementById("output").textContent = "";
            ...
            if (asset?.site?.roleAssignments?.[0]?.users) { // Checks if the users array exist
                ...
                for (const role of roles) { // Loop through each role/element of the array
                    ...
                    if (role.groups !== undefined) {
                        const name_arr = role.groups.split(','); // Name of groups are in a string seperated by commas, so we create an array of these names using split(',')
                        for (let item of name_arr) { // Loop through the array of names
                            let group = await readAsset("group", item); // Reads the group asset
                            let group_arr = group.group.users.split(';');
                            for (let member of group_arr) { // Loops through each member of the group
